...
/*   Updated: 2023/02/23 19:02:46 by moelkama         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check(char **map, char *file)
{
	...
	i = 0;
	c = ft_countchar(map, 'C');
	p = ft_countchar(map, 'P');
	e = ft_countchar(map, 'E');
	if (c == 0 || e != 1 || p != 1 || ft_countchar(map, 'F') > 1)
		return (ft_puterror(3, p, c, e), ft_free(map));
	while (map[i])
	{
		j = 0;
		while (map[i][j])
